...
import coil.load
import com.example.unsplashapi.databinding.ImageLayoutBinding
...
class ImageAdapter: RecyclerView.Adapter<ImageAdapter.ImageViewHolder>() {
   inner class ImageViewHolder(
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ImageViewHolder {
        return ImageViewHolder(ImageLayoutBinding.inflate(
            LayoutInflater.from(parent.context), parent,false
        ))
    }

    override fun onBindViewHolder(holder: ImageViewHolder, position: Int) {
       val currImag= differ.currentList[position]
        holder.binding.apply {
            tvDescription.text= currImag.description
            imageView.load(currImag.urls.full){
               crossfade(true)
                crossfade(1000)
